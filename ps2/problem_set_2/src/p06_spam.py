...
def predict_from_naive_bayes_model(model, matrix):
    ...
    return (
        matrix @ (np.log(phi_k_y1) - np.log(phi_k_y0)) + np.log(phi_y / (1 - phi_y))
        >= 0
    )
    # *** END CODE HERE ***
...
def get_top_five_naive_bayes_words(model, dictionary):
    ...
    inv_dic = {k: v for v, k in dictionary.items()}
    top_five_indicative_word_index = np.argsort(-(np.log(phi_i_y1) - np.log(phi_i_y0)))[
        :5
    ]
    return [inv_dic[i] for i in top_five_indicative_word_index]
    # *** END CODE HERE ***
...
def compute_best_svm_radius(
    ...
    for radius in radius_to_consider:
        accuracy = np.mean(
            svm.train_and_predict_svm(train_matrix, train_labels, val_matrix, radius)
            == val_labels
        )
...
def main():
    train_messages, train_labels = util.load_spam_dataset(
        r"D:\AI\ml\ps2\data\ds6_train.tsv"
    )
    val_messages, val_labels = util.load_spam_dataset(r"D:\AI\ml\ps2\data\ds6_val.tsv")
    test_messages, test_labels = util.load_spam_dataset(
        r"D:\AI\ml\ps2\data\ds6_test.tsv"
    )
